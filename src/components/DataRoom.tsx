    ...
    <section id={id} className="max-w-7xl mx-auto px-6 lg:px-12 py-16 border-t border-slate-200 scroll-mt-20">
      <div className="bg-[#0C2340] text-white p-8 lg:p-12 rounded-3xl shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-white/10">
          ...
          <button
            onClick={onOpenNda}
            className="flex items-center gap-2 px-6 py-3.5 bg-[#00A8B5] hover:bg-[#008893] text-white font-bold text-xs uppercase tracking-wider rounded-xl transition-all cursor-pointer shrink-0 shadow-md"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
          {DATA_ROOM_DOCS.map((doc) => (
            <div
              key={doc.id}
              className="bg-[#071629] p-6 rounded-2xl border border-slate-700/60 hover:border-[#00A8B5] transition-all flex flex-col justify-between group"
            >
              <div>
                {/* Category Badge & Protection */}
                <div className="flex items-center justify-between mb-4">
                  <span className="text-[9px] font-extrabold uppercase tracking-widest px-3 py-1 bg-[#0C2340] text-[#00A8B5] rounded-lg border border-[#00A8B5]/30">
                    {doc.category}
                  </span>
                  {doc.isProtected ? (
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-white group-hover:text-[#00A8B5] transition-colors">
                      {doc.title}
                    </h3>
              ...
              <div className="pt-4 border-t border-slate-700/60 flex items-center justify-between mt-4">
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-slate-300 hover:text-white font-medium cursor-pointer"
                >
                ...
                <button
                  onClick={onOpenNda}
                  className="flex items-center gap-1.5 px-4 py-2 bg-slate-800 hover:bg-[#00A8B5] text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer border border-slate-700"
                >
        ...
        {selectedDoc && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-[#0C2340] max-w-lg w-full p-8 text-white rounded-3xl border border-slate-700 relative space-y-6 shadow-2xl">
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  className="p-2 bg-slate-800 text-slate-300 hover:text-white rounded-full cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  onClick={() => setSelectedDoc(null)}
                  className="px-4 py-2.5 bg-slate-800 text-slate-300 hover:bg-slate-700 text-xs font-bold uppercase tracking-wider rounded-xl cursor-pointer"
                >
                  Fechar
                </button>
                <button
                  ...
                  className="px-5 py-2.5 bg-[#00A8B5] hover:bg-[#008893] text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all cursor-pointer shadow-md"
                >
