...
import { getSwotData } from '../data/dossierData';
import { ShieldAlert, CheckCircle2, AlertTriangle } from 'lucide-react';

interface SwotAndRiskProps {
  language?: Language;
}

export const SwotAndRisk: React.FC<SwotAndRiskProps> = ({ language = 'PT' as Language }) => {
      ...
      <div>
        <div className="max-w-3xl mb-8 sm:mb-12">
          <div className="text-[11px] font-extrabold uppercase tracking-widest text-[#00A8B5] mb-2">
            {isEn ? '05. Strategic Matrix' : '05. Matriz Estratégica'}
          </div>
          <h2 className="text-2xl sm:text-4xl font-extrabold text-[#0C2340] tracking-tight">
            {isEn ? 'SWOT Analysis' : 'Análise SWOT'}{' '}
            <span className="font-normal text-[#00A8B5]">
              {isEn ? '& Positioning' : '& Posicionamento'}
            </span>
          </h2>
          <p className="text-slate-600 text-sm mt-3 font-normal leading-relaxed">
            {isEn
              ? 'Rigorous internal and external positioning assessment of the investment in Besmart Boutique Residence.'
            ...
            <div>
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 className="text-[#0C2340] font-bold text-xs uppercase tracking-wider">
                  {isEn ? 'STRENGTHS' : 'FORÇAS (Strengths)'}
                </h3>
              </div>
          ...
          <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 className="text-[#0C2340] font-bold text-xs uppercase tracking-wider">
                  {isEn ? 'OPPORTUNITIES' : 'OPORTUNIDADES (Opps)'}
                </h3>
              </div>
              <ul className="space-y-3 text-xs text-slate-700 font-normal leading-relaxed">
          ...
          <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 className="text-[#0C2340] font-bold text-xs uppercase tracking-wider">
                  {isEn ? 'WEAKNESSES' : 'FRAQUEZAS (Weaknesses)'}
                </h3>
              </div>
              <ul className="space-y-3 text-xs text-slate-700 font-normal leading-relaxed">
